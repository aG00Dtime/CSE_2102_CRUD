...
class EmployeeManager(Tk):
    def __init__(self, access_level):
        ...
        self.tabs.add(self.tab1, text="Query Employee Database")
        ...
        self.enter_id_label = Label(self.tab3, text="Employee ID").grid(row=0, column=0,
        ...
        # button
        self.query_button_Employee = Button(self.tab3, width=20, text="Search", command=self.update_query).grid(row=1,
                                                                                                                column=1)
        ...
        # drop down selection box
        self.Employee_plan_label_2 = Label(self.tab3, text="Select Plan").grid(row=7, column=0, pady=10, padx=(300, 20),
                                                                               sticky=NW)

        self.Employee_plan_2 = Combobox(self.tab3, width=36, state='readonly')

        # query from db instead to values
        self.Employee_plan_2['values'] = self.db_get_plans()

        self.Employee_plan_2.grid(row=7, column=1, pady=10)

        # submit button
        self.submit_2 = Button(self.tab3, width=20, text="Update", command=self.update_record).grid(row=8, column=1)

        # TAB 4 #######################################################################################################
        self.enter_id_label_2 = Label(self.tab4, text="Employee ID").grid(row=0, column=0,
                                                                          pady=(50, 0), padx=(300, 20),
                                                                          sticky=NW)
        # Employee id
    ...
    def delete_record(self):
        ...
        employee_id = self.id_entry_2.get()

        db = db_conn()
        cur = db.cursor()
        # check if record exists in the database

        cur.execute(f'''select first_name,last_name from Employees where Employee_id = '{employee_id}' ''')
    ...
    def update_query(self):
        ...
        # fetch the record
        cur.execute(
            f'''select first_name,last_name,telephone,email,address,plan_id from Employees where employee_id= 
            '{employee_id}' ''')
        ...
        self.Employee_plan_2.current(plan_id)
    ...
    def update_record(self):
        ...
        if error_list:
            ...
            cur.execute(f''' 
            update Employees set first_name = '{first}',last_name='{last}',telephone='{phone}',address='{address1}',
            plan_id='{plan_id}',email='{email}' where employee_id='{employee_id}'
            ''')
    ...
    def db_query(self):
        ...
        cur.execute(f'''  select * from Employee_details where first_name ='{f_name}' and last_name ='{l_name}' ''')
    ...
    def submit_details(self):
        ...
        if error_list:
            ...
            cur.execute(f'''
            insert into Employees (first_name,last_name,telephone,email,address,plan_id) 
